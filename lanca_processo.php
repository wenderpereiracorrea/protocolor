...
<script>
    ...
    window.location.href = 'rel_capa_processo.php?nprocesso=<? echo $_POST[nprocesso]; ?>';
</script>
<?
} 
...
<BODY>
<center>
<form action="lanca_processo.php"  id="form1" method="POST" name="calform" target="_self">
...
<table width ="80%" align='center' border="1" cellpadding="1" cellspacing="2">
	<tr class="cabeçalho">
		<td colspan="4"><center>FORMAÇÃO DE PROCESSO</center></td>
	</tr>
	<? 
	...
	<? 
            // ************** Busca o último número de processo ******************
			$data_ano = date("Y");
            $sql1 = "select MAX(processo) as max from Processo where ano ='".$data_ano."'";
            $process = mysql_query($sql1) or die("Erro10: " . mysql_error());
            mysql_query("SET NAMES 'utf8'");
            mysql_query('SET character_set_connection=utf8');
            mysql_query('SET character_set_client=utf8');
            mysql_query('SET character_set_results=utf8');
...
<? // ********* CAMPOS DE PROCESSO  ********** ?>
	<tr>
		...
		<input type='text' name='dataent' size='10'  placeholder="xx/xx/xxxx" maxlength="10" class="cor-inativa" value= '<? if ($dataent=="") { echo date("d/m/Y");  } else {  echo $dataent; } ?>' onChange="javascript:verifica_data2()" onKeyPress="return txtBoxFormat(this, '99/99/9999', event);">
		 ...
		 <td class="caixaazul"><div align='right'>Próximo processo:&nbsp;</div></td>
                <td>
                    <input type="hidden" name="nlancamento" class="cor-inativa" value="<? echo $idprocesso; ?>" size="6" readonly title="Número de Lançamento">
					<input type="text" name="nprocesso" class="cor-inativa" value="<? echo $idprocesso; ?>" size="6" readonly title="Número do Processo">
                </td>
	</tr>
